...
PdmOverview::PdmOverview( QWidget* p ):QGraphicsView(p),d_subject( 0 ),d_mode(Idle)
{
...
void PdmOverview::resizeEvent ( QResizeEvent * e )
{
	onSceneRectChanged();
}

void PdmOverview::setObserved( PdmItemView* s )
{
	if( d_subject != s )
	{
		if( s )
...
void PdmOverview::onSceneRectChanged()
{
	if( scene() )
	{
...
void PdmOverview::paintEvent ( QPaintEvent * e )
{
	if( scene() )
	{
		QPainter p( viewport() );
...
QRectF PdmOverview::getSubjectRect() const
{
...
void PdmOverview::onScrolled()
{
	viewport()->update();
}

void PdmOverview::mousePressEvent ( QMouseEvent * e )
{
...
void PdmOverview::mouseMoveEvent ( QMouseEvent * e )
{
...
void PdmOverview::mouseReleaseEvent ( QMouseEvent * e )
{
	...
	}else if( d_mode == PrepareSelect && d_subject )
	{
		...
		d_subject->scrollBy( dx, dy );
	}else
		QGraphicsView::mouseReleaseEvent( e );
	d_mode = Idle;
}

void PdmOverview::mouseDoubleClickEvent ( QMouseEvent * e )
{
